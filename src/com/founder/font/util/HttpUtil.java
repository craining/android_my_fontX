...
import com.founder.font.response.BaseResponse;
...
public class HttpUtil {
	 ...
	 * @return
	 */
	public BaseResponse sendRequest(BaseResponse response, String url,
			List<NameValuePair> params, boolean isPost) {
		String result = null;
		try {
			if (isPost) {
				// result = postMethod(url, params);
				result = postMethod2(url, params);
			} else {
			...
			if (result != null) {
				...
				Log.e(TAG, " sendRequest " + url + " return null");
			}
	 ...
	 * @throws IOException
	 */
	private String postMethod2(String url, List<NameValuePair> params)
			throws IOException {

		HttpPost post = new HttpPost(
				"http://diy.foundertype.com/efont/GetMessage.aspx");
	...
	private String getMethod(String url, List<NameValuePair> params)
		...
		if (params != null) {
			...
			for (NameValuePair value : params) {
				...
				// if (!StringUtil.isEmpty(v, true)) {
				if (!StringUtil.isNull(v)) {
					buffer.append(URLEncoder.encode(value.getValue(),
							CHARSET_UTF8));
				}
				i++;
			}
